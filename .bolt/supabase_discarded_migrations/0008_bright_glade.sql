...
-- Comments policies
CREATE POLICY "comments_view_policy" ON report_comments
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "comments_insert_policy" ON report_comments
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id);

-- Attachments policies
CREATE POLICY "attachments_view_policy" ON report_attachments
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "attachments_insert_policy" ON report_attachments
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = uploaded_by);

-- History policies
CREATE POLICY "history_view_policy" ON report_histories
  FOR SELECT TO authenticated USING (true);